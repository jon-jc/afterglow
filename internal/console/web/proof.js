...
const integrationProof = (() => {
  ...
  function inlineContents(campaign) {
    ...
    return `<div class="panel-header"><div><h2>Live proof, right here</h2><p>${campaign ? "New runs use the selected campaign." : "New runs use an available demo campaign."} An unfinished run resumes its original identities.</p></div><button class="button primary" data-proof-run data-proof-campaign="${esc(campaign)}" ${running || !runtime.demo ? "disabled" : ""}>${running ? "Verifying…" : run && !done ? "Resume existing run" : "Run live proof · $0.02"}</button></div><div class="inline-proof-body"><p class="proof-status" role="status">${esc(message || "Run all six checks without leaving this page.")}${run ? `<br>Evidence campaign: ${esc(run.plan.campaign)} · ${run.steps.filter((s) => s.done).length}/6 stages verified` : ""}${storageWarning ? `<br>${esc(storageWarning)}` : ""}</p><p class="assurance-scope">Creates two $0.01 synthetic reservations. No real purchase. The page data refreshes after completion.</p>${run ? `<ol class="inline-proof-steps">${cards()}</ol><div class="inline-proof-evidence">${evidence()}</div><button class="button" data-proof-export>Export proof evidence ↓</button>` : ""}</div>`;
  }

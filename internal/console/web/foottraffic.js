...
const footTraffic = (() => {
  ...
  function update() {
    ...
    document
      ...
      .forEach((b) => {
        ...
        b.setAttribute("aria-busy", String(b.hasAttribute("data-traffic-refresh") ? loading : importing));
    ...
    if (select) select.disabled = loading || importing;
  ...
  async function ingest(replay) {
    ...
    try {
      ...
      remember();
      const result = await request(path("batches"), batch);
  ...
  function render() {
    queueMicrotask(() => {
      if (!report && !loading && !error) refresh(false);
      else update();
    });
    return (
      heading(
        ...
        `<button class="button" data-traffic-replay ${!attempts[selected] || loading || importing ? "disabled" : ""}>Replay last batch</button><button class="button" data-traffic-refresh ${loading || importing ? "disabled" : ""}>Refresh report ↻</button><button class="button primary" data-traffic-load ${loading || importing ? "disabled" : ""}>Load sample feed</button>`,
        "FOOT TRAFFIC",
      ) +
      `<div id="foot-traffic"><section class="panel traffic-samples"><div><label for="traffic-sample">Choose a sample feed</label><select id="traffic-sample" aria-describedby="traffic-sample-description" ${loading || importing ? "disabled" : ""}>${Object.entries(samples).map(([id, sample]) => `<option value="${id}" ${id === selected ? "selected" : ""}>${esc(sample[0])}</option>`).join("")}</select></div><div><p id="traffic-sample-description">${esc(samples[selected][1])}</p><small>Each sample keeps its own observations. Switching samples preserves previous imports.</small></div></section><p id="traffic-notice" class="traffic-notice" role="status" aria-live="polite">${esc(notice)}</p><div id="traffic-report">${content()}</div></div>`
    );
  }
  ...
  document.addEventListener("change", (e) => {
    if (e.target.id !== "traffic-sample" || loading || importing || !Object.hasOwn(samples, e.target.value)) return;
  ...
  return { render, refresh: () => { if (!importing) return refresh(); } };
})();
